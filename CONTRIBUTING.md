...
Thanks for your interest in `nk`! It's a small Go CLI, so getting started is
quick.
...
This builds the `nk` binary into `~/.local/bin/` (static, `CGO_ENABLED=0`).
...
suite, and `golangci-lint run --fix` (the config enables golines formatting).
...
accordingly. Release notes are generated from conventional commit messages, so
prefix commits with `feat:`, `fix:`, and so on.
